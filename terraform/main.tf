...
resource "null_resource" "init_cluster" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "cockroach sql --insecure --execute=\"SET CLUSTER SETTING kv.allocator.load_based_lease_rebalancing.enabled = false;\"",
      "cockroach sql --insecure --execute=\"SET CLUSTER SETTING kv.allocator.load_based_rebalancing = 0;\"",
